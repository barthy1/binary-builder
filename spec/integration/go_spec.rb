...
describe 'building a binary', :integration do
  context 'when go is specified' do
    before(:all) do
      ...
      @platform = (ENV['BINARY_BUILDER_PLATFORM'] == 'x86_64') ? "amd64" : ENV['BINARY_BUILDER_PLATFORM']
      @binary_tarball_location = File.join(Dir.pwd, "go1.6.3.linux-#{@platform}.tar.gz")
    end
    ...
    it 'builds the specified binary, tars it, and places it in your current working directory' do
      ...
      go_version_cmd = "./spec/assets/binary-exerciser.sh go1.6.3.linux-#{@platform}.tar.gz GOROOT=/tmp/binary-exerciser/go ./go/bin/go version"
